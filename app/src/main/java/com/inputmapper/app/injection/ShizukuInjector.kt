...
class ShizukuInjector : InputInjector {
    ...
    private fun execShizuku(cmd: String): String {
        return try {
            val process = Runtime.getRuntime().exec(arrayOf("sh", "-c", "rish -c '$cmd'"))
            val output = process.inputStream.bufferedReader().readText()
            process.waitFor()
            output
        } catch (e: Exception) {
            // Fallback to direct Shizuku service
            try {
                val service: IBinder? = Shizuku.peekService("com.inputmapper.app.shizuku.ShizukuInputService")
                if (service != null) {
                    execShellCommand(service, cmd)
                } else {
    ...
    private fun execShellCommand(service: IBinder, cmd: String): String {
        // Use Shizuku transact to execute shell commands
        val data = android.os.Parcel.obtain()
        val reply = android.os.Parcel.obtain()
        try {
            data.writeInterfaceToken("com.inputmapper.app.shizuku.IShizukuInputService")
    ...
    override fun keyEvent(keyCode: Int) {
        execShizuku("input keyevent $keyCode")
    }

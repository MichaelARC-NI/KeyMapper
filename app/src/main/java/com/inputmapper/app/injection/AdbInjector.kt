...
class AdbInjector(private var adbHost: String = "127.0.0.1", private var adbPort: Int = 5555) : InputInjector {
    ...
    private fun execAdb(cmd: String): String {
        return try {
            ...
            // Try direct socket ADB protocol
            execViaSocket(cmd)
        }
    }

    private fun execViaSocket(cmd: String): String {
        return try {
            val socket = Socket(adbHost, adbPort)
            val output = socket.getOutputStream()
            val input = BufferedReader(InputStreamReader(socket.getInputStream()))

            // ADB protocol: send shell command
            val message = "shell:$cmd"
            val msgBytes = message.toByteArray()
            val header = byteArrayOf(
                ((msgBytes.size) and 0xFF).toByte(),
                ((msgBytes.size shr 8) and 0xFF).toByte(),
                ((msgBytes.size shr 16) and 0xFF).toByte(),
                ((msgBytes.size shr 24) and 0xFF).toByte(),
    ...
    override fun keyEvent(keyCode: Int) {
        execAdb("input keyevent $keyCode")
    }
